...
\end{problem}
\begin{proof}
To show $\Null T^k = \Null T$, we first prove $\Null T^k = \Null T^{k + 1}$ for all $k\in\Z^+$.  Let $m\in\Z^+$.  Clearly, if $v\in\Null T^m$, then $v\in\Null T^{m+1}$, and hence $\Null T^m \subseteq\Null T^{m+1}$.  Next, suppose $v\in\Null T^{m + 1}$.  Then $T(T^m v)=0$, and hence $T^mv\in\Null T$.  By Problem 16, this implies $T^mv\in\Null T^\ast$, and by 7.7(a) we have $T^m\in(\Range T)^\perp$.  Since of course $T^mv\in\Range T$ as well, we must have $T^mv = 0$.  Thus $v \in \Null T^m$, and therefore $\Null T^{m+1} \subseteq \Null T^m$.  Thus $\Null T^m = \Null T^{m + 1}$.  Since $m$ was arbitrary, this implies $\Null T^k = \Null T$ for all $k\in\Z^+$, as desired.\\
