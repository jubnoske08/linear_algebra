...
\indent $(b \Rightarrow c)$  Suppose $V = N + R$.  By Theorem 2.43, we have 
\begin{equation}
\dim(N + R) = \dim N + \dim R - \dim ( N \cap R), \label{eq15}
\end{equation}
and hypothesis, the LHS of Equation \ref{eq15} equals $\dim V$.  Hence we have
\begin{equation}
...
\end{equation}
Combining Equations \ref{eq16} and \ref{eq17}, we have $\dim(N\cap R) = 0$, and hence $N\cap R =\{0\}$.\\
...
\end{equation}
By another application of the Fundamental Theorem of Linear Maps, we have 
\begin{equation}
\dim N + \dim R = \dim V. \label{eq19}
\end{equation}  
Hence, combining Equations \ref{eq18} and \ref{eq19}, we conclude $\dim V = \dim(N + R)$, and hence $V = N + R$.  Since $N\cap R = \{0\}$ by hypothesis, this sum is direct.
\end{proof}
\end{document}